...
def fill_template(template_path: str | None, values: dict[str, str]) -> Document:
    ...
    _process(doc)
    add_disclaimer_header(doc)
    add_disclaimer_footer(doc)
    return doc


def add_disclaimer_header(doc: Document) -> None:
    """
    Insert a 'DRAFT — TEMPLATE ONLY' notice at the start of the document body.
    Styled bold, centered, red font, with a bottom border.
    """
    from docx.shared import Pt, RGBColor
    from docx.enum.text import WD_ALIGN_PARAGRAPH

    para = doc.add_paragraph()
    # Move the new paragraph to the very beginning of the document body
    doc.element.body.insert(0, para._element)

    run = para.add_run(
        "DRAFT — TEMPLATE ONLY — NOT A LEGAL DOCUMENT — FOR REVIEW PURPOSES ONLY"
    )
    run.bold = True
    run.font.size = Pt(10)
    ...
    # Small spacing below the header paragraph
...
def add_disclaimer_footer(doc: Document) -> None:
    """
    Add a disclaimer notice to every section footer in the document.
    Styled italic, 8pt, centered, gray.
    """
    from docx.shared import Pt, RGBColor
    from docx.enum.text import WD_ALIGN_PARAGRAPH
    ...
    footer_text = (
        f"Generated by EquityForm (beta). This document is a template only and does not "
        f"constitute legal advice. Do not execute without review by a licensed attorney. "
        f"© EquityForm {year}"
    )
